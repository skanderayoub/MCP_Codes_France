...
# How to run (React)
Perfect! Let’s make your **FastAPI AI agents actually use the MCP tools** (like `search_code`, `get_article_by_id`, etc.) instead of just being a plain LLM wrapper.

This means:

* We’ll **start your MCP server in parallel** (just like before).
* The AI agent will load all MCP tools dynamically and use them to answer user queries.
* Your `/ask` endpoint will now allow the model to call those tools when reasoning.

---

### 🛠 How It Works

Your old `client.py` used:

* `BasicMCPClient` → connect to MCP server
* `McpToolSpec` → wrap server tools
* `FunctionAgent` + `Context` → agent reasoning loop

We’ll port that into the FastAPI server so your API endpoint runs an MCP-powered agent.

---

### 🔥 Updated `server_ai.py`
...
async def ask(req: AskRequest):
    """Ask the LLM a question about the code using MCP tools."""
    try:
...
# (Optional) keep your direct endpoints
@app.get("/search")
async def search_code(query: str, max_results: int = 10):
    """Call MCP tool directly for raw search."""
    # We'll call MCP search directly (no agent)
    client = BasicMCPClient(MCP_SERVER_URL)
    await client.start()
    tool = await client.get_tool("search_code")
    result = await tool.call(query=query, max_results=max_results)
...
* ✅ `/ask` now spins up a **FunctionAgent** with **live MCP tools** loaded.
* ✅ The agent can reason, call `search_code`, `get_article_by_id`, etc., and produce **tool-augmented answers**.
* ✅ You can still use `/search` or other endpoints to hit MCP directly.
...
1. Start the **MCP server** (`server.py`):
   ...
   ```bash
   python server.py --server_type sse --json_path path/to/json.json
   ```

   This will serve tools at `http://127.0.0.1:8000/sse`.

2. Start the **FastAPI server**:

   ```bash
   uvicorn server_ai:app --reload --port 9000
   ```

   Now your API is on `http://127.0.0.1:9000`.
